...
export default async function ClaimDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <div>
        <Link href="/app/claims" className="text-sm text-orange-600 hover:underline">← Claims</Link>
        ...
        <p className="text-sm text-slate-500">Geopend door {c.opened_by} · {dateTimeNL(c.created_at)}{c.amount ? ` · geclaimd ${eur(c.amount)}` : ""}</p>
      </div>
      ...
      <section className="ph-card p-4">
        ...
        {c.resolution && (
          <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm">
            <span className="font-semibold text-slate-700">Uitkomst:</span> {c.resolution}
          </div>
        )}
        <Link href={`/app/shipments/${c.shipment_id}`} className="mt-3 inline-block text-sm text-orange-600 hover:underline">Bekijk zending & custody →</Link>
      </section>

      <section className="ph-card p-4">
        <SectionTitle sub="Communicatie over deze claim">Berichten</SectionTitle>
        <div className="space-y-2">
          {messages.length === 0 && <p className="text-sm text-slate-400">Nog geen berichten.</p>}
          {messages.map((m, i) => (
      ...
      {isOps && open && (
        <section className="ph-card p-4">
          <SectionTitle sub="Toekennen = terugboeking naar de afzender via de betaal-adapter">Beoordeling (ops)</SectionTitle>
          <div className="mb-3 rounded-lg bg-orange-50 p-3 text-xs text-orange-800">
            <div className="mb-1 font-semibold">🤖 AI-conceptantwoord (suggestie):</div>
            {aiDraft}
          </div>
          <form action={resolveClaimAction} className="space-y-3">
            <input type="hidden" name="claim_id" value={id} />
            <textarea name="resolution" rows={2} placeholder="Onderbouwing…"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-orange-500" />
            <div className="flex flex-wrap gap-2">
              <button name="outcome" value="REFUND" className="ph-btn ph-btn-primary text-sm">Toekennen & terugboeken</button>
              <button name="outcome" value="INVESTIGATE" className="ph-btn ph-btn-ghost text-sm">In onderzoek</button>
              <button name="outcome" value="REJECT" className="ph-btn ph-btn-ghost text-sm text-rose-600">Afwijzen</button>
            </div>
          </form>
        </section>
      )}
