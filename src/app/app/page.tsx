  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Hallo {user.firstName} 👋</h1>
          <p className="text-sm text-slate-500">{user.roleName} · PakketHub NL–SR pilot</p>
        </div>
        <InstallAppButton label="Installeer als app" />
      </div>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard label="Zendingen" value={c.shipments ?? 0} />
        <StatCard label="Afgeleverd" value={c.delivered ?? 0} />
        <StatCard label="In beoordeling" value={c.review ?? 0} />
        <StatCard label="Corridor" value="NL→SR" hint="Pilot actief" />
      </div>

      {isSender && (
        <section>
          <div className="mb-3 flex items-center justify-between">
            <SectionTitle sub="Je laatste aangemaakte zendingen">Mijn zendingen</SectionTitle>
            <Link href="/app/shipments/new" className="ph-btn ph-btn-primary text-sm">+ Pakket versturen</Link>
          </div>
          {myShipments.length === 0 ? (
            <EmptyState icon="📦" title="Nog geen zendingen">
              Maak je eerste declaratie aan. We checken meteen of het via een reiziger mag.
            </EmptyState>
          ) : (
              ...
              {myShipments.map((s) => (
                <Link key={s.id} href={`/app/shipments/${s.id}`} className="flex items-center justify-between gap-3 p-3 hover:bg-slate-50">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-slate-500">Naar {s.recipient_city} · deadline {dateNL(s.deadline)} · {eur(s.value)}</div>
                  </div>
      ...
      {isTraveler && (
        <section>
          <div className="mb-3 flex items-center justify-between">
            <SectionTitle sub="Zendingen die je op je reis kunt meenemen">Open voor vervoer</SectionTitle>
            <Link href="/app/marketplace" className="text-sm font-medium text-orange-600 hover:underline">Alles bekijken →</Link>
          </div>
          {openForTravel.length === 0 ? (
            <EmptyState icon="🧳" title="Geen open zendingen">Kom later terug of publiceer een rit.</EmptyState>
          ) : (
            <div className="ph-card divide-y divide-slate-100">
              {openForTravel.map((s) => (
                <Link key={s.id} href={`/app/marketplace/${s.id}`} className="flex items-center justify-between gap-3 p-3 hover:bg-slate-50">
                  <div>
                    <div className="font-mono text-sm font-semibold text-slate-800">{s.reference}</div>
                    <div className="text-xs text-slate-500">{s.corridor} · {s.kg ?? "?"} kg · deadline {dateNL(s.deadline)}</div>
                  </div>
                  <span className="ph-btn ph-btn-ghost text-xs">Bekijk & bied</span>
                </Link>
              ))}
            </div>
          )}
      ...
      {isOps && (
        <section>
          <SectionTitle sub="Zendingen die menselijke beoordeling nodig hebben">Beoordelingswachtrij</SectionTitle>
          {reviewQueue.length === 0 ? (
            <EmptyState icon="✅" title="Wachtrij leeg">Geen zendingen in beoordeling of hold.</EmptyState>
          ) : (
            <div className="ph-card divide-y divide-slate-100">
              {reviewQueue.map((s) => (
