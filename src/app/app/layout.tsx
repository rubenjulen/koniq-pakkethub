...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Ongelezen berichten-indicatie (simpel: berichten na last_read_at van de gebruiker).
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
      ...
      <aside className="sticky top-0 hidden h-screen flex-col border-r border-slate-200 bg-white lg:flex">
        <div className="border-b border-slate-100 p-4"><Logo /></div>
        ...
        <div className="border-t border-slate-100 p-3">
          ...
          <div className="mt-2 flex gap-1">
            <Link href="/app/account" className="flex-1 rounded-lg px-2 py-1.5 text-center text-xs text-slate-500 hover:bg-slate-50">Account</Link>
            <Link href="/app/notifications" className="flex-1 rounded-lg px-2 py-1.5 text-center text-xs text-slate-500 hover:bg-slate-50">🔔 Meldingen</Link>
          </div>
          <form action={doLogout} className="mt-1">
            <button className="w-full rounded-lg px-3 py-2 text-left text-sm text-slate-500 hover:bg-slate-50">Uitloggen</button>
          </form>
        </div>
      </aside>
      ...
      <div className="flex flex-col">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur lg:hidden">
          <Logo size={28} />
          <form action={doLogout}><button className="text-sm text-slate-500">Uitloggen</button></form>
        </header>
