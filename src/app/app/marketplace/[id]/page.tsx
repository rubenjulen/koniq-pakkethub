...
export default async function MarketplaceDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <div>
        <Link href="/app/marketplace" className="text-sm text-orange-600 hover:underline">← {m.market.title}</Link>
        <div className="mt-1 flex items-center gap-2">
          <h1 className="font-mono text-xl font-bold text-slate-900">{s.reference}</h1>
          <EligibilityBadge decision={s.eligibility} />
        </div>
        <p className="text-sm text-slate-500">{s.corridor_name} · {d.to} {s.recipient_city}, {s.recipient_country}</p>
      </div>
      ...
      <section className="ph-card p-4">
        <SectionTitle sub={`${s.declared_weight_kg ?? "?"} kg · ${d.deadline} ${dateNL(s.deadline)} · ${eur(s.value)}`}>{d.content}</SectionTitle>
      ...
      <section className="ph-card p-4">
        <SectionTitle sub={d.offer_sub}>{myOffer ? d.adjust_bid : d.place_bid}</SectionTitle>
        <form action={makeOfferAction} className="space-y-3">
          <input type="hidden" name="shipment_id" value={s.id} />
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="text-xs font-medium text-slate-600">{d.your_price}</span>
            ...
            {trips.length > 0 && (
              <label className="block">
                <span className="text-xs font-medium text-slate-600">{d.link_trip}</span>
          ...
          <label className="block">
            <span className="text-xs font-medium text-slate-600">{d.msg_sender}</span>
            <textarea name="message" rows={2} defaultValue={myOffer?.message ?? ""}
              placeholder={d.msg_ph}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500" />
          </label>
          <button className="ph-btn ph-btn-primary">{myOffer ? d.update_bid : d.place_chat}</button>
        </form>
      </section>
    </div>
  );
