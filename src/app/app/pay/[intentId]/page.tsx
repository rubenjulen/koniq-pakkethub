    ...
    <div className="mx-auto max-w-md space-y-5">
      <div className="ph-card overflow-hidden">
        <div className="flex items-center justify-between bg-slate-900 px-5 py-3 text-white">
          <span className="font-semibold">Beveiligde betaling</span>
          <Chip tone="ok">🔒 Sandbox</Chip>
        </div>
        <div className="p-5">
          <div className="mb-4 rounded-lg bg-amber-50 p-3 text-xs text-amber-800 ring-1 ring-amber-200">
            <strong>Simulatie.</strong> Dit is de demo-betaalpagina van <em>{PROVIDERS.payments.name}</em>.
            Er wordt geen echt geld verplaatst — kies hieronder een uitkomst om de flow te testen.
            Bij livegang staat hier de kassa van de echte betaalprovider.
          </div>

          <div className="text-sm text-slate-500">Betaling voor</div>
          <div className="font-mono text-lg font-bold text-slate-900">{b?.reference}</div>
          <div className="text-sm text-slate-500">Bezorging in {b?.recipient_city}</div>

          <dl className="mt-4 space-y-1 border-t border-slate-100 pt-3 text-sm">
            <div className="flex justify-between"><dt className="text-slate-500">Vervoersvergoeding</dt><dd>{eur(price)}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Servicekosten PakketHub</dt><dd>{eur(fee)}</dd></div>
            <div className="mt-1 flex justify-between border-t border-slate-100 pt-2 text-base font-bold text-slate-900">
              <dt>Totaal</dt><dd>{eur(intent.amount_eur)}</dd>
            </div>
          </dl>

          <div className="mt-3 rounded-lg bg-orange-50 p-3 text-xs text-orange-800">
            🛡️ Je betaling wordt <strong>in bewaring (escrow)</strong> gehouden en pas vrijgegeven aan de
            reiziger na bewijs van levering.
          </div>

          {failed && (
            <div className="mt-3 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 ring-1 ring-rose-200">
              De betaling is mislukt (simulatie). Probeer opnieuw.
            </div>
          )}

          {done ? (
            <div className="mt-4 rounded-lg bg-orange-50 px-3 py-2 text-center text-sm font-medium text-orange-700">✓ Betaling geslaagd</div>
          ) : (
            <div className="mt-4 grid gap-2">
              <form action={confirmPaymentAction}>
                <input type="hidden" name="intent_id" value={intentId} />
                <input type="hidden" name="outcome" value="success" />
                <button className="ph-btn ph-btn-primary w-full">Betaal {eur(intent.amount_eur)} (simuleer geslaagd)</button>
              </form>
              <form action={confirmPaymentAction}>
                <input type="hidden" name="intent_id" value={intentId} />
                <input type="hidden" name="outcome" value="fail" />
                <button className="ph-btn ph-btn-ghost w-full text-slate-500">Simuleer mislukte betaling</button>
              </form>
            </div>
          )}
        </div>
      </div>
      <p className="text-center text-xs text-slate-400">Provider: {PROVIDERS.payments.name} · modus {PROVIDERS.payments.mode}</p>
    </div>
  );
}
